...
def train_dqn():
    ...
    for episode in range(num_episodes):
        ...
        if episode % 100 == 0 and episode > 0:
            ...
            emoji = "🔥" if win_rate > 60 else "📈" if win_rate > 40 else "💪" if win_rate > 25 else "🎲"
            print(f"{emoji} Ep {episode:5d} | Avg: {avg_gain:7.1f}$ | WR: {win_rate:5.1f}% | ε: {eps_threshold:.3f} | Acts: {actions_dist}")
            action_counts = {0: 0, 1: 0, 2: 0}
